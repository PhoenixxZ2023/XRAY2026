...
# --- PASSO 3b: REMOVE USUÁRIOS SSH EXPIRADOS ---
if systemctl is-active --quiet turbonet-dropbear 2>/dev/null && [ -s "$expired_nicks_file" ]; then
    while IFS= read -r enick; do
        [ -n "$enick" ] || continue
        if id "$enick" &>/dev/null 2>/dev/null; then
            userdel "$enick" 2>/dev/null || true
        fi
